...
DEFAULT_TRAIN = Path("data/processed/feature_eng_train.csv")
DEFAULT_EVAL = Path("data/processed/feature_eng_eval.csv")
DEFAULT_OUT = Path("models/xgb_best_model.pkl")


def _normalize_tracking_uri(tracking_uri: str) -> str:
    """Convert local filesystem paths to a file URI for MLflow."""
    uri = str(tracking_uri)
    if uri.startswith(("file:", "http:", "https:", "databricks:", "sqlite:", "postgresql:", "mysql:", "mssql:")):
        return uri
...
def tune_model(
    ...
    with mlflow.start_run(run_name="best_xgb_model"):
        ...
        try:
            mlflow_xgboost.log_model(best_model, "model")
        except Exception as exc:
            print(f"⚠️ MLflow XGBoost logging skipped: {exc}")
